...
test.describe('Site Integrity', () => {
  test('Crawl site and check for broken links', async ({ page, request }) => {
      ...
      try {
        ...
        for (const link of links) {
          try {
            ...
            try {
              const linkResponse = await request.get(`${baseUrl}${normalizedUrl}`);
              ...
              if (linkStatusCode >= 400) {
                ...
                console.log(`❌ Broken link found: ${normalizedUrl} (${linkStatusCode})`);
              } else {
              ...
              brokenLinks.push(linkResult);
              console.log(`❌ Link request failed: ${normalizedUrl} - ${linkError}`);
            }
    ...
    if (brokenLinks.length > 0) {
      ...
      brokenLinks.forEach(link => {
        console.log(`  - ${link.url}: ${link.statusCode} ${link.statusText} ${link.error ? `(${link.error})` : ''}`);
      });
    }

    // Assert that no broken links were found
    expect(brokenLinks.length, `Found ${brokenLinks.length} broken links:\n${
      brokenLinks.map(link => `  - ${link.url}: ${link.statusCode} ${link.statusText} ${link.error ? `(${link.error})` : ''}`).join('\n')
    }`).toBe(0);
  ...
  test('Verify homepage loads correctly', async ({ page }) => {
    ...
    expect(hasHeader || hasMain, 'Page should have header/nav or main content area').toBe(true);
    
    console.log('✅ Homepage loads correctly');
  });

  test('Verify all critical pages are accessible', async ({ page, request }) => {
    ...
    for (const path of criticalPaths) {
      ...
      expect(response.status(), `Critical path ${path} should be accessible`).toBeLessThan(400);
