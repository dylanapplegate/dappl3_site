#!/usr/bin/env node

/**
 * Build script to generate tokens.json from Tailwind config
 * This script extracts design tokens from tailwind.config.js and converts them
 * to a Style Dictionary compatible format for use with Figma Tokens Studio
 */

import { createRequire } from "module";
import fs from "fs";
import path from "path";
import { fileURLToPath } from "url";

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const require = createRequire(import.meta.url);

// Load and resolve Tailwind config
async function loadResolvedTailwindTheme() {
  try {
    // Import the Tailwind config
    const configModule = await import(
      `file://${path.resolve(__dirname, "../tailwind.config.js")}`
    );
    const tailwindConfig = configModule.default;

    // For Tailwind v4, we need to manually build the theme with defaults
    // Since resolveConfig doesn't exist, we'll combine defaults with extensions
    const theme = {
      colors: {
        // Core Tailwind colors (minimal essential set)
        inherit: "inherit",
        current: "currentColor",
        transparent: "transparent",
        black: "#000000",
        white: "#ffffff",
        slate: {
          50: "#f8fafc",
          100: "#f1f5f9",
          200: "#e2e8f0",
          300: "#cbd5e1",
          400: "#94a3b8",
          500: "#64748b",
          600: "#475569",
          700: "#334155",
          800: "#1e293b",
          900: "#0f172a",
          950: "#020617",
        },
        gray: {
          50: "#f9fafb",
          100: "#f3f4f6",
          200: "#e5e7eb",
          300: "#d1d5db",
          400: "#9ca3af",
          500: "#6b7280",
          600: "#4b5563",
          700: "#374151",
          800: "#1f2937",
          900: "#111827",
          950: "#030712",
        },
        zinc: {
          50: "#fafafa",
          100: "#f4f4f5",
          200: "#e4e4e7",
          300: "#d4d4d8",
          400: "#a1a1aa",
          500: "#71717a",
          600: "#52525b",
          700: "#3f3f46",
          800: "#27272a",
          900: "#18181b",
          950: "#09090b",
        },
        red: {
          50: "#fef2f2",
          100: "#fee2e2",
          200: "#fecaca",
          300: "#fca5a5",
          400: "#f87171",
          500: "#ef4444",
          600: "#dc2626",
          700: "#b91c1c",
          800: "#991b1b",
          900: "#7f1d1d",
          950: "#450a0a",
        },
        blue: {
          50: "#eff6ff",
          100: "#dbeafe",
          200: "#bfdbfe",
          300: "#93c5fd",
          400: "#60a5fa",
          500: "#3b82f6",
          600: "#2563eb",
          700: "#1d4ed8",
          800: "#1e40af",
          900: "#1e3a8a",
          950: "#172554",
        },
        green: {
          50: "#f0fdf4",
          100: "#dcfce7",
          200: "#bbf7d0",
          300: "#86efac",
          400: "#4ade80",
          500: "#22c55e",
          600: "#16a34a",
          700: "#15803d",
          800: "#166534",
          900: "#14532d",
          950: "#052e16",
        },
        yellow: {
          50: "#fefce8",
          100: "#fef9c3",
          200: "#fef08a",
          300: "#fde047",
          400: "#facc15",
          500: "#eab308",
          600: "#ca8a04",
          700: "#a16207",
          800: "#854d0e",
          900: "#713f12",
          950: "#422006",
        },
        purple: {
          50: "#faf5ff",
          100: "#f3e8ff",
          200: "#e9d5ff",
          300: "#d8b4fe",
          400: "#c084fc",
          500: "#a855f7",
          600: "#9333ea",
          700: "#7e22ce",
          800: "#6b21a8",
          900: "#581c87",
          950: "#3b0764",
        },
        pink: {
          50: "#fdf2f8",
          100: "#fce7f3",
          200: "#fbcfe8",
          300: "#f9a8d4",
          400: "#f472b6",
          500: "#ec4899",
          600: "#db2777",
          700: "#be185d",
          800: "#9d174d",
          900: "#831843",
          950: "#500724",
        },
        // Add custom colors from the config
        ...(tailwindConfig.theme?.extend?.colors || {}),
      },
      spacing: {
        px: "1px",
        0: "0px",
        0.5: "0.125rem",
        1: "0.25rem",
        1.5: "0.375rem",
        2: "0.5rem",
        2.5: "0.625rem",
        3: "0.75rem",
        3.5: "0.875rem",
        4: "1rem",
        5: "1.25rem",
        6: "1.5rem",
        7: "1.75rem",
        8: "2rem",
        9: "2.25rem",
        10: "2.5rem",
        11: "2.75rem",
        12: "3rem",
        14: "3.5rem",
        16: "4rem",
        20: "5rem",
        24: "6rem",
        28: "7rem",
        32: "8rem",
        36: "9rem",
        40: "10rem",
        44: "11rem",
        48: "12rem",
        52: "13rem",
        56: "14rem",
        60: "15rem",
        64: "16rem",
        72: "18rem",
        80: "20rem",
        96: "24rem",
        ...(tailwindConfig.theme?.extend?.spacing || {}),
      },
      fontSize: {
        xs: ["0.75rem", { lineHeight: "1rem" }],
        sm: ["0.875rem", { lineHeight: "1.25rem" }],
        base: ["1rem", { lineHeight: "1.5rem" }],
        lg: ["1.125rem", { lineHeight: "1.75rem" }],
        xl: ["1.25rem", { lineHeight: "1.75rem" }],
        "2xl": ["1.5rem", { lineHeight: "2rem" }],
        "3xl": ["1.875rem", { lineHeight: "2.25rem" }],
        "4xl": ["2.25rem", { lineHeight: "2.5rem" }],
        "5xl": ["3rem", { lineHeight: "1" }],
        "6xl": ["3.75rem", { lineHeight: "1" }],
        "7xl": ["4.5rem", { lineHeight: "1" }],
        "8xl": ["6rem", { lineHeight: "1" }],
        "9xl": ["8rem", { lineHeight: "1" }],
        ...(tailwindConfig.theme?.extend?.fontSize || {}),
      },
      fontFamily: {
        sans: ["ui-sans-serif", "system-ui", "sans-serif"],
        serif: ["ui-serif", "Georgia", "serif"],
        mono: ["ui-monospace", "SFMono-Regular", "monospace"],
        ...(tailwindConfig.theme?.extend?.fontFamily || {}),
      },
      borderRadius: {
        none: "0px",
        sm: "0.125rem",
        DEFAULT: "0.25rem",
        md: "0.375rem",
        lg: "0.5rem",
        xl: "0.75rem",
        "2xl": "1rem",
        "3xl": "1.5rem",
        full: "9999px",
        ...(tailwindConfig.theme?.extend?.borderRadius || {}),
      },
      boxShadow: {
        sm: "0 1px 2px 0 rgb(0 0 0 / 0.05)",
        DEFAULT:
          "0 1px 3px 0 rgb(0 0 0 / 0.1), 0 1px 2px -1px rgb(0 0 0 / 0.1)",
        md: "0 4px 6px -1px rgb(0 0 0 / 0.1), 0 2px 4px -2px rgb(0 0 0 / 0.1)",
        lg: "0 10px 15px -3px rgb(0 0 0 / 0.1), 0 4px 6px -4px rgb(0 0 0 / 0.1)",
        xl: "0 20px 25px -5px rgb(0 0 0 / 0.1), 0 8px 10px -6px rgb(0 0 0 / 0.1)",
        "2xl": "0 25px 50px -12px rgb(0 0 0 / 0.25)",
        inner: "inset 0 2px 4px 0 rgb(0 0 0 / 0.05)",
        none: "none",
        ...(tailwindConfig.theme?.extend?.boxShadow || {}),
      },
    };

    return theme;
  } catch (error) {
    console.error("Error loading Tailwind config:", error);
    throw error;
  }
}

// Convert values to Style Dictionary format, handling different value types
function convertToTokenFormat(value, key) {
  // Handle fontSize arrays [size, { lineHeight: '...' }]
  if (key.includes("fontSize") && Array.isArray(value) && value.length === 2) {
    return {
      value: value[0],
      lineHeight: value[1].lineHeight || value[1],
    };
  }

  // Handle fontFamily arrays
  if (key.includes("fontFamily") && Array.isArray(value)) {
    return {
      value: value.join(", "),
    };
  }

  // Handle regular values
  return {
    value: value,
  };
}

// Convert Tailwind theme object to tokens format recursively
function convertThemeToTokens(obj, parentKey = "") {
  const result = {};

  for (const [key, value] of Object.entries(obj)) {
    const currentKey = parentKey ? `${parentKey}.${key}` : key;

    if (value && typeof value === "object" && !Array.isArray(value)) {
      // Recursively process nested objects
      result[key] = convertThemeToTokens(value, currentKey);
    } else {
      // Convert leaf values to token format
      result[key] = convertToTokenFormat(value, currentKey);
    }
  }

  return result;
}

async function buildTokens() {
  try {
    console.log("🚀 Building tokens from Tailwind config...");

    // Load resolved Tailwind theme (includes all defaults + extensions)
    const theme = await loadResolvedTailwindTheme();

    // Convert theme to tokens format
    const tokens = {
      $themes: [
        {
          id: "light",
          name: "Light Theme",
          selectedTokenSets: {
            global: "enabled",
          },
          $figmaStyleReferences: {},
        },
      ],
      global: convertThemeToTokens(theme),
    };

    // Write the tokens file
    const outputPath = path.resolve(__dirname, "../tokens.json");
    fs.writeFileSync(outputPath, JSON.stringify(tokens, null, 2));

    console.log("✅ Tokens successfully generated at:", outputPath);
    console.log(`🎨 Generated tokens for: ${Object.keys(theme).join(", ")}`);
    console.log(`📊 Color variants: ${Object.keys(theme.colors || {}).length}`);
    console.log(
      `📐 Spacing values: ${Object.keys(theme.spacing || {}).length}`,
    );
    console.log(`🔤 Font sizes: ${Object.keys(theme.fontSize || {}).length}`);
  } catch (error) {
    console.error("❌ Error building tokens:", error);
    process.exit(1);
  }
}

// Run the build process
buildTokens();
